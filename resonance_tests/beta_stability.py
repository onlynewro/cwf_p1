"""Refitting and stability diagnostics for β parameters."""
from __future__ import annotations

import csv
import math
from dataclasses import dataclass
from pathlib import Path
from typing import Iterable, List, Optional

import matplotlib

matplotlib.use("Agg")
import matplotlib.pyplot as plt
import numpy as np

from .utils import (
    RationalApproximation,
    best_rational_approximation,
    ensure_directory,
    weighted_linear_regression,
)


@dataclass
class BetaBinResult:
    """Result of the β stability refit for a single redshift bin."""

    bin_label: str
    beta0: float
    beta1: float
    beta0_uncertainty: float
    beta1_uncertainty: float
    chi2: float
    approximation: Optional[RationalApproximation]
    ratio_alert: bool = False


@dataclass
class BetaStabilityResult:
    """Aggregate results for β stability."""

    bins: List[BetaBinResult]
    artifact_path: Path
    table_path: Path

    def rational_alerts(self) -> List[BetaBinResult]:
        """Return bins that triggered a low-order rational approximation alert."""

        return [bin_result for bin_result in self.bins if bin_result.approximation is not None]

    def ratio_lock_alerts(self) -> List[BetaBinResult]:
        """Return bins whose ratios triggered a consecutive lock-in alert."""

        return [bin_result for bin_result in self.bins if bin_result.ratio_alert]


def _format_bin_label(index: int, centers: np.ndarray, *, edges: Optional[np.ndarray] = None) -> str:
    if edges is not None and index < len(edges) - 1:
        return f"z∈[{edges[index]:.3g}, {edges[index + 1]:.3g})"
    label = centers[index]
    if isinstance(label, (int, np.integer)):
        return f"bin-{int(label)}"
    return f"bin-{index}"


def run_beta_stability(
    z: Iterable[float],
    sigma_a: Iterable[float],
    beta_predictions: Iterable[float],
    weights: Optional[Iterable[float]] = None,
    *,
    bin_edges: Optional[Iterable[float]] = None,
    max_denominator: int = 16,
    approximation_tolerance: float = 5e-3,
    output_dir: Path | str = Path("resonance_artifacts"),
    ratio_q_max: int = 10,
    ratio_tau: float = 2.0,
    ratio_min_consecutive: int = 2,
) -> BetaStabilityResult:
    """Refit β(σ(a)) relations inside each redshift bin."""

    z = np.asarray(list(z), dtype=float)
    sigma_a = np.asarray(list(sigma_a), dtype=float)
    beta_predictions = np.asarray(list(beta_predictions), dtype=float)
    if not (z.shape == sigma_a.shape == beta_predictions.shape):
        raise ValueError("Inputs must share the same shape")

    if weights is None:
        weights_arr = np.ones_like(z)
    else:
        weights_arr = np.asarray(list(weights), dtype=float)
        if weights_arr.shape != z.shape:
            raise ValueError("weights must match z")
        weights_arr = np.clip(weights_arr, 0.0, np.inf)

    if bin_edges is not None:
        edges = np.asarray(list(bin_edges), dtype=float)
        if edges.ndim != 1 or edges.size < 2:
            raise ValueError("bin_edges must provide at least two edges")
        bin_indices = np.digitize(z, edges) - 1
        bin_centers = 0.5 * (edges[:-1] + edges[1:])
    else:
        edges = None
        bin_centers, bin_indices = np.unique(z, return_inverse=True)

    bin_results: List[BetaBinResult] = []
    bin_labels: List[str] = []

    for idx in range(len(bin_centers)):
        mask = bin_indices == idx
        if not np.any(mask):
            continue
        x = sigma_a[mask]
        y = beta_predictions[mask]
        w = weights_arr[mask]
        params, cov, chi2 = weighted_linear_regression(x, y, w)
        beta0, beta1 = params
        beta0_unc = math.sqrt(max(cov[0, 0], 0.0))
        beta1_unc = math.sqrt(max(cov[1, 1], 0.0))
        ratio = beta1 / (2.0 * math.pi) if beta1 != 0 else 0.0
        approximation = best_rational_approximation(ratio, max_denominator, approximation_tolerance)
        label = _format_bin_label(idx, bin_centers, edges=edges)
        bin_labels.append(label)
        bin_results.append(
            BetaBinResult(
                bin_label=label,
                beta0=float(beta0),
                beta1=float(beta1),
                beta0_uncertainty=float(beta0_unc),
                beta1_uncertainty=float(beta1_unc),
                chi2=float(chi2),
                approximation=approximation,
            )
        )

    if not bin_results:
        raise ValueError("No populated bins available for β stability analysis")

    slopes = np.array([result.beta1 for result in bin_results])
    slope_unc = np.array([result.beta1_uncertainty for result in bin_results])
    ratio_flags = low_order_ratio_alert(
        slopes,
        slope_unc,
        q_max=ratio_q_max,
        tau=ratio_tau,
        min_consecutive=ratio_min_consecutive,
    )
    for result, flag in zip(bin_results, ratio_flags):
        result.ratio_alert = bool(flag)

    output_dir_path = ensure_directory(output_dir)
    output_path = output_dir_path / "beta_slopes.png"
    table_path = output_dir_path / "beta_slopes.csv"

    fig, ax = plt.subplots(figsize=(8, 4))
    ax.errorbar(
        np.arange(len(slopes)),
        slopes,
        yerr=[result.beta1_uncertainty for result in bin_results],
        fmt="o",
        label="β₁",
    )
    for idx, result in enumerate(bin_results):
        if result.approximation is not None:
            ax.annotate(
                result.approximation.as_string(),
                (idx, result.beta1),
                textcoords="offset points",
                xytext=(0, 10),
                ha="center",
                fontsize=8,
            )
        if result.ratio_alert:
            ax.scatter(idx, result.beta1, color="tab:red", marker="s", s=40, label="ratio alert" if idx == 0 else "")
    ax.set_xticks(np.arange(len(bin_labels)))
    ax.set_xticklabels(bin_labels, rotation=45, ha="right")
    ax.set_ylabel("β₁")
    ax.set_title("β stability across redshift bins")
    ax.legend()
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(output_path, dpi=200)
    plt.close(fig)

    with table_path.open("w", newline="") as fp:
        writer = csv.writer(fp)
        writer.writerow([
            "bin",
            "beta0",
            "beta0_unc",
            "beta1",
            "beta1_unc",
            "chi2",
            "rational",
            "ratio_alert",
        ])
        for result in bin_results:
            writer.writerow(
                [
                    result.bin_label,
                    f"{result.beta0:.6g}",
                    f"{result.beta0_uncertainty:.6g}",
                    f"{result.beta1:.6g}",
                    f"{result.beta1_uncertainty:.6g}",
                    f"{result.chi2:.6g}",
                    result.approximation.as_string() if result.approximation else "",
                    int(result.ratio_alert),
                ]
            )

    return BetaStabilityResult(bin_results, output_path, table_path)


def _ratio_with_uncertainty(b1_i: float, s_i: float, b1_j: float, s_j: float) -> tuple[float, float]:
    r = b1_i / b1_j
    denom_i = max(abs(b1_i), 1e-12)
    denom_j = max(abs(b1_j), 1e-12)
    sr = abs(r) * math.sqrt((s_i / denom_i) ** 2 + (s_j / denom_j) ** 2)
    return r, sr


def low_order_ratio_alert(
    beta1_list: Iterable[float],
    sigma_list: Iterable[float],
    *,
    q_max: int = 10,
    tau: float = 2.0,
    min_consecutive: int = 2,
) -> List[bool]:
    """Detect consecutive lock-in to low-order rational ratios."""

    beta_arr = np.asarray(list(beta1_list), dtype=float)
    sigma_arr = np.asarray(list(sigma_list), dtype=float)
    n = len(beta_arr)
    if n == 0:
        return []
    if n < 3:
        return [False] * n

    close = np.zeros(n, dtype=bool)
    for i in range(n - 1):
        r, sr = _ratio_with_uncertainty(beta_arr[i + 1], sigma_arr[i + 1], beta_arr[i], sigma_arr[i])
        if not np.isfinite(sr) or sr <= 0:
            continue
        hit = False
        for q in range(1, q_max + 1):
            for p in range(1, q_max + 1):
                target = p / q
                if abs(r - target) <= tau * sr:
                    hit = True
                    break
            if hit:
                break
        close[i + 1] = hit

    flags = [False] * n
    run = 0
    for i in range(n):
        if close[i]:
            run += 1
            if run + 1 >= min_consecutive:
                flags[i] = True
        else:
            run = 0
    return flags
