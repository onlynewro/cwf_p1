"""Command line interface for the cosmology workflow."""
...
from src.data_loaders import BAOData, CMBData, SNData
from src.models import LCDMModel, RDFitWrapper, SevenDModel

warnings.filterwarnings("ignore")

if sys.platform == "darwin":  # macOS compatibility for multiprocessing
    mp.set_start_method("fork", force=True)


def main():
    parser = argparse.ArgumentParser(description="7D Cosmology Joint Fitting")
    parser.add_argument("--model", choices=["LCDM", "7D", "both"], default="both", help="Model to fit")
    parser.add_argument("--bao-file", type=str, default=None, help="BAO data file (JSON)")
    parser.add_argument("--sn-file", type=str, default=None, help="Supernova data file")
    parser.add_argument("--use-cmb", action="store_true", help="Include CMB constraints")
    parser.add_argument(
        "--use-default-bao",
        action="store_true",
        help="Load the packaged DESI BAO catalogue when no file is supplied",
    )
    parser.add_argument("--rd-mode", choices=["fixed", "fit"], default="fixed", help="Sound horizon mode")
    parser.add_argument("--disable-bao-cov", action="store_true", help="Ignore supplied BAO covariance matrices")
    parser.add_argument(
        "--no-proxy", action="store_true", help="Omit the legacy QSO proxy point from the default BAO catalogue"
    )
    parser.add_argument("--drop-lya-dh", action="store_true", help="Remove the Lyα DH/rd point from the final fit")
    parser.add_argument(
        "--diagnose-lya-dh",
        action="store_true",
        help="Temporarily inspect residuals without the Lyα DH/rd point before fitting",
    )
    parser.add_argument("--workers", type=int, default=-1, help="Number of workers for parallelization (-1 for all CPUs)")
    parser.add_argument("--maxiter", type=int, default=1000, help="Maximum iterations for optimization")
    parser.add_argument("--output", type=str, default="fit_results.json", help="Output file for results")
    ...
    print("=== 7D Cosmology Joint Fitting ===")
    ...
    if bao_requested:
        ...
        print(
            f"  BAO: {bao_data.count_observables()} observables from {len(bao_data.data)} entries loaded"
        )
    ...
    if args.sn_file:
        try:
            ...
            print(
                f"      columns: z='{columns.get('z')}', mu='{columns.get('mu')}', sigma='{columns.get('sigma')}'"
            )
            ...
            with datasets["bao"].temporarily_drop("DESI Lyα GCcomb", "DH_over_rd") as dropped:
                ...
                if dropped:
                    ...
                    diag_pull = datasets["bao"].print_residual_table(
                        fid_lcdm, fid_params, rd_value=147.0, title="(Lyα DH/rd temporarily removed)"
                    )
    ...
    print(
        "  BAO (final): "
        f"{final_bao_count} observables from {len(datasets['bao'].data) if datasets['bao'] else 0} entries in use"
    )
    ...
    from scipy.optimize import differential_evolution  # imported lazily to avoid heavy import at module load
    ...
    for model in models_to_fit:
        ...
        obj = partial(total_chi2, datasets=datasets, model=model, rd_mode=args.rd_mode)
        ...
        try:
            ...
            if not de_res.success:
                ...
                try:
                    ...
                    errors_payload = {
                        key: _finite_float_or_none(value) for key, value in cov_result.errors.items()
                    }
            ...
            best_fit_parameters = {
                name: float(value) for name, value in zip(model.param_names, de_res.x)
            }
    ...
    if "LCDM" in results and "7D" in results:
        if results["LCDM"].get("success") and results["7D"].get("success"):
            ...
            for model_name in ["LCDM", "7D"]:
                ...
                print(
                    f"{model_name:<10} {stats['chi2']:<10.3f} "
