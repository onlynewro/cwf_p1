"""Planck 2018 CMB distance prior loader."""
...
class CMBData:
    ...
    PLANCK2018_COV = np.array(
        [
            ...
            [7.2910e-03, 3.6510e00, -2.1980e-03],
    ...
    def __init__(
        self,
        mean_vector: Optional[Sequence[float]] = None,
        covariance: Optional[Sequence[Sequence[float]]] = None,
    ) -> None:
        self.mean = np.array(
            mean_vector if mean_vector is not None else self.PLANCK2018_MEAN, dtype=float
        )
        self.cov = np.array(
            covariance if covariance is not None else self.PLANCK2018_COV, dtype=float
        )
    ...
    def _extract_omega_b_h2(self, model, params: Sequence[float]):
    ...
    def _theory_vector(self, model, params: Sequence[float]):
        h = params[0]
        Om = params[1]
        H0 = h * 100.0
    ...
    def chi2(self, model, params: Sequence[float]) -> float:
